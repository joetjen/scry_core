defmodule Scry.Core.Query.From do
  @moduledoc """
  `Scry.Core.Query.from/2`'s own implementation -- kept out of
  `Scry.Core.Query` itself so that module's own moduledoc (documenting
  the struct/Layer 1) doesn't have to also carry this macro's own
  keyword-clause dispatch. `Scry.Core.Query.from/2` is a one-line
  ...
  this) -- Layer 1's own functions (`Scry.Core.Query.new/1`, `where/2`,
  ...
  **Nested `from`** (a `select:` map value that's itself a `from` call
  ...
  `Scry.Core.Executor`'s own `project_item/8` always projects it under
  ...
  def build({:in, _, [var_ast, source_ast]}, opts, outer_vars, env) when is_list(opts) do
    ...
    base = quote do: Scry.Core.Query.new(List.wrap(unquote(source_ast)))
  ...
  defp clause_call(:where, value_ast, vars, _source_ast, env) do
    ...
    quote do: Scry.Core.Query.where(unquote(escaped))
  end

  defp clause_call(:having, value_ast, vars, _source_ast, env) do
    escaped = Escape.escape_predicate(value_ast, vars, env)
    quote do: Scry.Core.Query.having(unquote(escaped))
  end
  ...
  defp clause_call(:group_by, value_ast, vars, _source_ast, env) when is_list(value_ast) do
    ...
    quote do: Scry.Core.Query.group_by(unquote(escaped))
  end
  ...
  defp clause_call(:order_by, value_ast, vars, _source_ast, env) when is_list(value_ast) do
    ...
    quote do: Scry.Core.Query.order_by(unquote(escaped))
  end

  defp clause_call(:distinct, value_ast, _vars, _source_ast, _env) do
    quote do: Scry.Core.Query.distinct(unquote(value_ast))
  end

  defp clause_call(:limit, value_ast, _vars, _source_ast, _env) do
    quote do: Scry.Core.Query.limit(unquote(value_ast))
  end

  defp clause_call(:offset, value_ast, _vars, _source_ast, _env) do
    quote do: Scry.Core.Query.offset(unquote(value_ast))
  end
  ...
  defp clause_call(:select, {:%{}, _, pairs}, vars, source_ast, env) do
    ...
    quote do: Scry.Core.Query.select(unquote(items))
  end
  ...
  # `Scry.Core.Executor.project_item/8`'s own `{:ok, List.last(nested.
    ...
    unless key_name == inner_qualifier do
      raise ArgumentError,
            "`select:` map key `#{key_name}:` doesn't match the nested `from`'s own source " <>
              "(`#{inner_qualifier}`) -- Scry.Core.Executor always projects a nested query under " <>
