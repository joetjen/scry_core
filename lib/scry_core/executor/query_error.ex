defmodule ScryCore.Executor.QueryError do
  @moduledoc """
  Raised while pulling from a `%ScryCore.Cursor{}` returned by `ScryCore.
  ...
  **Why a raise, not a return value, for this specific case:** `run/4`'s
  ...
  lazy end to end (`ScryCore.Cursor`'s own moduledoc has the full
  ...
  **This is invisible at every *internal* boundary that already needs a
  ...
  row, either side of `UNION`/`INTERSECT`/`EXCEPT`) -- `ScryCore.Executor`'s
