defmodule ScryCore.EngineBehaviour do
  ...
  `ScryCore.Cursor` is the intended way to consume whatever comes
  ...
  @doc "Fetches every row for `source` (a dotted path, e.g. `[\"orders\"]`) from `conn` -- any `Enumerable.t()` of `row()`, not necessarily a materialized list (`ScryCore.Cursor`'s own moduledoc has the full reasoning)."
