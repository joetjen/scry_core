defmodule ScryCore.WithCycleCheck do
  @moduledoc """
  Static cycle detection over a document's own `WITH` bindings
  (`ScryCore.Query.t/0`'s own `with_bindings`, lang_spec.md §9), invoked
  once from `ScryCore.Actions`' own `handle_rule(:document, ...)` before
  `ScryCore.parse/1` ever returns.

  A `WITH` reference is resolved at *execution* time (`ScryCore.Executor`
  ...
  `ScryCore.Executor.run/3` recurse into itself forever the moment either
  ...
  `ScryCore.FragmentResolver`'s own cycle check already established for
  fragment spreads.
  """
