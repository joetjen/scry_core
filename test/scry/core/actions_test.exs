defmodule Scry.Core.ActionsTest do
  ...
  setup_all do
    ...
    # and analyzes on its own now (Scry.Core.GrammarComposeTest covers
  ...
  defp run(grammar, query), do: Grammar.VM.run(grammar, query, Scry.Core.Actions, nil)
  ...
  test "an atom literal, tagged rather than a real Elixir atom", %{grammar: g} do
    ...
    # Not `:active` -- see Scry.Core.Actions' own handle_token(:ATOM, ...)
...
line two""" { name }))
  ...
  test "distinct is syntactically permitted on any call's argument, not just count's", %{
    ...
    # Scry.Core.ExecutorTest) -- confirms `sum(distinct x)` at least
  ...
  describe "TYPE declarations (lang_spec.md §7) -- parsed shape; consumption is Scry.Core.TypeCheck's own job" do
    ...
    test "a combined query (UNION) still gets its own type_decls attached", %{grammar: g} do
      assert {:ok, %Scry.Core.CombinedQuery{} = q} =
               run(
                 g,
