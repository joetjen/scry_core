defmodule ScryCore.FragmentResolver do
  @moduledoc """
  Expands `{:spread, name}` placeholders (`ScryCore.Actions`' own
  ...
  Invoked once, by `ScryCore.Actions`' own `handle_rule(:document, ...)`,
  ...
  struct `ScryCore.parse/1` hands back, exactly as if the spread's own
  ...
  same raw-tuple convention `ScryCore.Actions`' own `handle_token`
  ...
  module's own moduledoc):
    ...
    - `{:duplicate_fragment, name}` -- caught earlier, by
      `ScryCore.Actions`' own `build_fragment_map/1`, before this module
